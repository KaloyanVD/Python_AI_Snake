...
# font = pygame.font.Font('arial', 25) -> slower for some reason?
...
class SnakeGameAI:
    ...
    def play_step(self, action):
        ...
        self._move(action)  # updates the head of the snake
        self.snake.insert(0, self.head)

        # check if game is over
        reward = 0
        game_over = False
        # check if we hit something or if the snake does nothing for long time
    ...
    def is_collision(self, point=None):
        if point is None:
            point = self.head
        # check if it hits boundaries
        if point.x > self.w - BLOCK_SIZE or point.x < 0 or point.y > self.h - BLOCK_SIZE or point.y < 0:
            return True

        # check if it hits itself. Ignore first pos since it's the head itself
